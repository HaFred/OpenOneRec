...
class RolloutCorrectionConfig(BaseConfig):
    """Configuration for rollout correction and importance sampling.
    
    Args:
        bypass_mode (bool): If True, use bypass mode (old_log_probs = rollout_log_probs).
                           If False, recompute old_log_probs as proximal anchor.
        enable_is_weights (bool): Whether to apply importance sampling weights to policy loss.
        is_threshold (Optional[float]): Upper threshold for IS weights. None = disabled.
        is_threshold_lower (Optional[float]): Lower threshold for IS weights.
        is_level (str): Aggregation level: "token", "sequence", or "geometric".
        is_mode (str): Bounding mode: "truncate" or "mask".
        is_veto_threshold (Optional[float]): Per-token veto threshold for catastrophic outliers.
    """
    _frozen_fields = ["bypass_mode", "enable_is_weights", "is_threshold", "is_level", "is_mode"]
    bypass_mode: bool = False
    enable_is_weights: bool = False
...
class AlgoConfig(BaseConfig):
    ...
    Args:
        ...
        policy_loss_type (str): Type of policy loss: "ppo", "gpg", "clip_cov", etc.
    """
