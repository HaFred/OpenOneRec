...
```text
+----------------------------------+
| run_grpo.sh / main_onerec_ppo    |
+----------------------------------+
                 |
                 v
+----------------------------------+
| OneRecTaskRunner.run()           |
+----------------------------------+
                 |
                 v
+----------------------------------+
| Build role_worker_mapping        |
| (Megatron strategy)              |
+----------------------------------+
                 |
                 v
...
+----------------------------------+
                 ...
                 v                                                      v
+----------------------------------+                +----------------------------------+
| init_workers()                   |                | fit() training loop              |
+----------------------------------+                +----------------------------------+
                 |                                                      |
                 v                                                      v
+----------------------------------+                +----------------------------------+
| NVMegatronRayWorkerGroup         |                | Generate sequences               |
| (spawn colocated workers)        |                | (WG or AgentLoopManager)        |
+----------------------------------+                +----------------------------------+
                 |                                                      |
                 v                                                      v
+----------------------------------+                +----------------------------------+
| Actor/Critic/Ref/RM init_model() |               | Reward path                       |
+----------------------------------+                | reward_fn / rm_wg.compute_rm...  |
                 |                                  +----------------------------------+
                 v                                                      |
+-----------------------------------------------+                      v
| MegatronWorker._init_hf_config_and_tf_config  |      +-----------------------------------------------+
| (..., use_mbridge=True)                        |      | compute_log_prob + compute_ref_log_prob       |
+-----------------------------------------------+      | + compute_values                               |
                 |                                      +-----------------------------------------------+
                 v                                                      |
+----------------------------------+                                    v
| AutoBridge.from_config(hf_config)|                    +----------------------------------+
| bridge=configured                |                    | compute_advantage                |
+----------------------------------+                    +----------------------------------+
          |                  |                                          |
          v                  v                                          v
+-------------------+  +-------------------+            +----------------------------------+
| bridge.get_model  |  | bridge.load_weights|           | critic_wg.update_critic          |
+-------------------+  +-------------------+            | (if enabled)                     |
                                                         +----------------------------------+
                                                                          |
                                                                          v
...
From "Actor/Critic/Ref/RM init_model()":
                 |
                 v
          +-------------------+
          | rollout.name ?    |
          +-------------------+
             |                               |
             | two_stage + sync              | vllm / sglang async
             v                               v
+------------------------------------------+ +----------------------------------+
| OneRecMegatronActorRolloutRefWorker      | | AsyncActorRolloutRefWorker       |
| ._build_rollout()                        | +----------------------------------+
+------------------------------------------+                 |
             |                                              v
             |                       +----------------------------------+
             ...
             |                                              v
             |                       +-----------------------------------------------+
             |                       | async_llm_server_* actors + AgentLoopWorker   |
             |                       +-----------------------------------------------+
             v
+-----------------------------------------------+
| OneRecvLLMRollout                              |
| (stage1 generate + stage2 beam_search)         |
+-----------------------------------------------+
             |
             v
+-----------------------------------------------+
